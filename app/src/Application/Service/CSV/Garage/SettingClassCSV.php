...
class SettingClassCSV implements CSVInterface
{
    use MigrationCSV;
    ...
    {
        $class  = $this->entityManager->getRepository(SettingClass::class)->findOneBy(['slug' => $datas['SettingClass']]);
        $garage = $this->findGarage($datas);
        $garage->setSettingClass($class);
    ...
    {
        return $this->parameter->get('csv.file.garage.setting.class');
    }
    ...
    {
        return $this->parameter->get('csv.header.garage.setting.class');
    }
}
