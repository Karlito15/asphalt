...
#[AsCommand(
    name: 'asphalt:csv:garage:status:control',
    description: "Toutes les données pour le Garage",
    aliases: ['asphalt-csv-garage-status-control'],
    hidden: false,
)]
class StatusControlCommand extends Command
{
    ...
    protected static string $title    = '::::: Garages Status Control Datas :::::';

    protected static string $help     = '';

    private static string $database   = 'csv.folders.database.datas';
    private static string $file       = 'csv.file.garage.status-control';
    private static string $header     = 'csv.header.garage.status-control';
    ...
    private function import(string $folder, SymfonyStyle $io): void
    {
        ...
        if ($fs->exists($filepath)) {
            ...
            foreach ($records as $record) {
                ...
                ### Create Entity
                $entity = App::createStatusControl($this->entityManager, $record);
