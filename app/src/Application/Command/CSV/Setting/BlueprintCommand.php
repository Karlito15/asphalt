...
use App\Infrastructure\Data\Setting\Blueprint;
...
#[AsCommand(
    name: 'asphalt:csv:setting:blueprint',
    description: "Toutes les données pour les Blueprints",
    aliases: ['asphalt-csv-setting-blueprint'],
    hidden: false,
)]
class BlueprintCommand extends Command
{
    ...
    protected static string $title    = '::::: Settings Blueprints Datas :::::';

    protected static string $help     = '';

    private static string $database   = 'csv.folders.database.datas';
    private static string $file       = 'csv.file.setting.blueprint';
    private static string $header     = 'csv.header.setting.blueprint';
...
//        $logger     = $this->logger;
        ...
        if ($choice === 'import') {
            ...
            $io->newLine(2);
            $io->success("Les données pour les Blueprints sont importées");
            $result = true;
        }
        ...
        if ($choice === 'export') {
            ...
            $io->newLine(2);
            $io->success("Les données pour les Blueprints sont exportées");
            $result = true;
        }
    ...
    private function import(string $folder, SymfonyStyle $io): void
    {
        ...
        if ($fs->exists($filepath)) {
            ...
            foreach ($records as $record) {
                ...
                ### Create Entity
                $entity = Blueprint::create($record);
