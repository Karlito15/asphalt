...
use App\Infrastructure\Data\Setting\Brand;
...
#[AsCommand(
    name: 'asphalt:csv:setting:brand',
    description: "Toutes les données pour les Brands",
    aliases: ['asphalt-csv-setting-brand'],
    hidden: false,
)]
class BrandCommand extends Command
{
    ...
    protected static string $title    = '::::: Settings Brands Datas :::::';

    protected static string $help     = '';

    private static string $database   = 'csv.folders.database.datas';
    private static string $file       = 'csv.file.setting.brand';
    private static string $header     = 'csv.header.setting.brand';
...
//        $logger     = $this->logger;
        ...
        if ($choice === 'import') {
            ...
            $io->newLine(2);
            $io->success("Les données pour les Brands sont importées");
            $result = true;
        }
        ...
        if ($choice === 'export') {
            ...
            $io->newLine(2);
            $io->success("Les données pour les Brands sont exportées");
            $result = true;
        }
