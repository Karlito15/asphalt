...
use App\Infrastructure\Data\Race\Region;
...
#[AsCommand(
    name: 'asphalt:csv:race:region',
    description: "Toutes les données pour les Regions",
    aliases: ['asphalt-csv-race-region'],
    hidden: false,
)]
class RegionCommand extends Command
{
    ...
    protected static string $title    = '::::: Races Regions Datas :::::';

    protected static string $help     = '';

    private static string $database   = 'csv.folders.database.datas';
    private static string $file       = 'csv.file.race.region';
    private static string $header     = 'csv.header.race.region';
...
//        $logger     = $this->logger;
        ...
        if ($choice === 'import') {
            ...
            $io->newLine(2);
            $io->success("Les données pour les Regions sont importées");
            $result = true;
        }
        ...
        if ($choice === 'export') {
            ...
            $io->newLine(2);
            $io->success("Les données pour les Regions sont exportées");
            $result = true;
        }
