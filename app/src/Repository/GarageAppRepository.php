...
class GarageAppRepository extends ServiceEntityRepository
{
    ...
    public function search(?GarageDTO $search): array
    {
        // ORDER
//        dump($search->order);
//        if (is_null($search->order) or $search->order = 'carOrder') {
//            $qb = $this->queryGarage()
//                ->addOrderBy('settingClass.classOrder', 'ASC')
//                ->addOrderBy('g.carOrder', 'ASC')
//            ;
//        }
//        if ($search->order = 'statOrder') {
//            $qb = $this->queryGarage()
//                ->addOrderBy('settingClass.classOrder', 'ASC')
//                ->addOrderBy('g.statOrder', 'ASC')
//            ;
//        }
        $qb = $this->queryGarage()
            ->addOrderBy('settingClass.classOrder', 'ASC')
        ...
        if ($search->getGameUpdate()) {
            $qb = $qb->andWhere('g.gameUpdate = :gameUpdate')->setParameter('gameUpdate', $search->garage->getGameUpdate());
        }
        if ($search->getBrand()) {
            // dump($search->brand->getName());
