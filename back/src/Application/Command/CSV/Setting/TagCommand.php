...
use App\Infrastructure\Data\Setting\Tag;
...
#[AsCommand(
    name: 'asphalt:csv:setting:tag',
    description: "Toutes les données pour les Tags",
    aliases: ['asphalt-csv-setting-tag'],
    hidden: false,
)]
class TagCommand extends Command
{
    ...
    protected static string $title    = '::::: Settings Tags Datas :::::';

    protected static string $help     = '';

    private static string $database   = 'csv.folders.database.datas';
    private static string $file       = 'csv.file.setting.tag';
    private static string $header     = 'csv.header.setting.tag';
...
//        $logger     = $this->logger;
        ...
        if ($choice === 'import') {
            ...
            $io->newLine(2);
            $io->success("Les données pour les Tags sont importées");
            $result = true;
        }
        ...
        if ($choice === 'export') {
            ...
            $io->newLine(2);
            $io->success("Les données pour les Tags sont exportées");
            $result = true;
        }
