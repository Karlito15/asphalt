...
use App\Infrastructure\Data\Setting\UnitPrice;
...
#[AsCommand(
    name: 'asphalt:csv:setting:unit',
    description: "Toutes les données pour les UnitPrices",
    aliases: ['asphalt-csv-setting-unit'],
    hidden: false,
)]
class UnitPriceCommand extends Command
{
    use ConfigureCommand, InitializeCommand, InteractCommand, QuestionCommand;
    use CSVCommand, FileSystemCommand;

    protected static string $title    = '::::: Settings Unit Prices Datas :::::';

    protected static string $help     = '';

    private static string $database   = 'csv.folders.database.datas';
    private static string $file       = 'csv.file.setting.unit-price';
    private static string $header     = 'csv.header.setting.unit-price';
...
//        $logger     = $this->logger;
        ...
        if ($choice === 'import') {
            ...
            $io->newLine(2);
            $io->success("Les données pour les UnitPrices sont importées");
            $result = true;
        }
        ...
        if ($choice === 'export') {
            ...
            $io->newLine(2);
            $io->success("Les données pour les UnitPrices sont exportées");
            $result = true;
        }
    ...
    private function import(string $folder, SymfonyStyle $io): void
    {
        ...
        if ($fs->exists($filepath)) {
            ...
            foreach ($records as $record) {
                ...
                ### Create Entity
                $entity = UnitPrice::create($record);
