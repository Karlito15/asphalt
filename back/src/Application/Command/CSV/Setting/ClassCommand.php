...
use App\Infrastructure\Data\Setting\Classes;
...
#[AsCommand(
    name: 'asphalt:csv:setting:class',
    description: "Toutes les données pour les Classes",
    aliases: ['asphalt-csv-setting-class'],
    hidden: false,
)]
class ClassCommand extends Command
{
    ...
    protected static string $title    = '::::: Settings Classes Datas :::::';

    protected static string $help     = '';

    private static string $database   = 'csv.folders.database.datas';
    private static string $file       = 'csv.file.setting.class';
    private static string $header     = 'csv.header.setting.class';
...
//        $logger     = $this->logger;
        ...
        if ($choice === 'import') {
            ...
            $io->newLine(2);
            $io->success("Les données pour les Classes sont importées");
            $result = true;
        }
        ...
        if ($choice === 'export') {
            ...
            $io->newLine(2);
            $io->success("Les données pour les Classes sont exportées");
            $result = true;
        }
