...
use App\Infrastructure\Data\Setting\Level;
...
#[AsCommand(
    name: 'asphalt:csv:setting:level',
    description: "Toutes les données pour les Levels",
    aliases: ['asphalt-csv-setting-level'],
    hidden: false,
)]
class LevelCommand extends Command
{
    ...
    protected static string $title    = '::::: Settings Levels Datas :::::';

    protected static string $help     = '';

    private static string $database   = 'csv.folders.database.datas';
    private static string $file       = 'csv.file.setting.level';
    private static string $header     = 'csv.header.setting.level';
...
//        $logger     = $this->logger;
        ...
        if ($choice === 'import') {
            ...
            $io->newLine(2);
            $io->success("Les données pour les Levels sont importées");
            $result = true;
        }
        ...
        if ($choice === 'export') {
            ...
            $io->newLine(2);
            $io->success("Les données pour les Levels sont exportées");
            $result = true;
        }
