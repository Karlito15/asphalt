...
use App\Infrastructure\Data\Race\Mode;
...
#[AsCommand(
    name: 'asphalt:csv:race:mode',
    description: "Toutes les données pour les Modes",
    aliases: ['asphalt-csv-race-mode'],
    hidden: false,
)]
class ModeCommand extends Command
{
    ...
    protected static string $title    = '::::: Races Modes Datas :::::';

    protected static string $help     = '';

    private static string $database   = 'csv.folders.database.datas';
    private static string $file       = 'csv.file.race.mode';
    private static string $header     = 'csv.header.race.mode';
...
//        $logger     = $this->logger;
        ...
        if ($choice === 'import') {
            ...
            $io->newLine(2);
            $io->success("Les données pour les Modes sont importées");
            $result = true;
        }
        ...
        if ($choice === 'export') {
            ...
            $io->newLine(2);
            $io->success("Les données pour les Modes sont exportées");
            $result = true;
        }
