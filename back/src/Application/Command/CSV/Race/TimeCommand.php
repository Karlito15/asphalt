...
use App\Infrastructure\Data\Race\Time;
...
#[AsCommand(
    name: 'asphalt:csv:race:time',
    description: "Toutes les données pour les Times",
    aliases: ['asphalt-csv-race-time'],
    hidden: false,
)]
class TimeCommand extends Command
{
    ...
    protected static string $title    = '::::: Races Times Datas :::::';

    protected static string $help     = '';

    private static string $database   = 'csv.folders.database.datas';
    private static string $file       = 'csv.file.race.time';
    private static string $header     = 'csv.header.race.time';
...
//        $logger     = $this->logger;
        ...
        if ($choice === 'import') {
            ...
            $io->newLine(2);
            $io->success("Les données pour les Times sont importées");
            $result = true;
        }
        ...
        if ($choice === 'export') {
            ...
            $io->newLine(2);
            $io->success("Les données pour les Times sont exportées");
            $result = true;
        }
