...
use App\Infrastructure\Data\Race\Season;
...
#[AsCommand(
    name: 'asphalt:csv:race:season',
    description: "Toutes les données pour les Seasons",
    aliases: ['asphalt-csv-race-season'],
    hidden: false,
)]
class SeasonCommand extends Command
{
    ...
    protected static string $title    = '::::: Races Seasons Datas :::::';

    protected static string $help     = '';

    private static string $database   = 'csv.folders.database.datas';
    private static string $file       = 'csv.file.race.season';
    private static string $header     = 'csv.header.race.season';
...
//        $logger     = $this->logger;
        ...
        if ($choice === 'import') {
            ...
            $io->newLine(2);
            $io->success("Les données pour les Seasons sont importées");
            $result = true;
        }
        ...
        if ($choice === 'export') {
            ...
            $io->newLine(2);
            $io->success("Les données pour les Seasons sont exportées");
            $result = true;
        }
