...
namespace App\Application\Command\CSV\Race;
...
use App\Infrastructure\Data\Race\Track;
...
#[AsCommand(
    name: 'asphalt:csv:race:track',
    description: "Toutes les données pour les Tracks",
    aliases: ['asphalt-csv-race-track'],
    hidden: false,
)]
class TrackCommand extends Command
{
    ...
    protected static string $title    = '::::: Races Tracks Datas :::::';

    protected static string $help     = '';

    private static string $database   = 'csv.folders.database.datas';
    private static string $file       = 'csv.file.race.track';
    private static string $header     = 'csv.header.race.track';
    private static string $folderName = 'races';
...
//        $logger     = $this->logger;
        ...
        if ($choice === 'import') {
            ...
            $io->newLine(2);
            $io->success("Les données pour les Tracks sont importées");
            $result = true;
        }
        ...
        if ($choice === 'export') {
            ...
            $io->newLine(2);
            $io->success("Les données pour les Tracks sont exportées");
            $result = true;
        }
    ...
    private function import(string $folder, SymfonyStyle $io): void
    {
        ...
        if ($fs->exists($filepath)) {
            ...
            foreach ($records as $record) {
                ...
                ### Create Entity
                $entity = Track::create($this->entityManager, $record);
