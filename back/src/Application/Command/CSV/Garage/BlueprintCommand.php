...
#[AsCommand(
    name: 'asphalt:csv:garage:blueprint',
    description: "Toutes les données pour le Garage",
    aliases: ['asphalt-csv-garage-blueprint'],
    hidden: false,
)]
class BlueprintCommand extends Command
{
    ...
    protected static string $title    = '::::: Garages Blueprints Datas :::::';

    protected static string $help     = '';

    private static string $database   = 'csv.folders.database.datas';
    private static string $file       = 'csv.file.garage.blueprint';
    private static string $header     = 'csv.header.garage.blueprint';
    ...
    private function import(string $folder, SymfonyStyle $io): void
    {
        ...
        if ($fs->exists($filepath)) {
            ...
            foreach ($records as $record) {
                ...
                ### Create Entity
                $entity = App::createBlueprint($this->entityManager, $record);
