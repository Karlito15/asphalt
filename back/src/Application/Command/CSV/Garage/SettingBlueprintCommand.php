...
#[AsCommand(
    name: 'asphalt:csv:garage:setting:blueprint',
    description: "Toutes les données pour le Garage",
    aliases: ['asphalt-csv-garage-setting-blueprint'],
    hidden: false,
)]
class SettingBlueprintCommand extends Command
{
    ...
    private static string $file       = 'csv.file.garage.setting.blueprint';
    private static string $header     = 'csv.header.garage.setting.blueprint';
    ...
    private function import(string $folder, SymfonyStyle $io): void
    {
        ...
        if ($fs->exists($filepath)) {
            ...
            foreach ($records as $record) {
                ...
                ### Update Entity
                $entity = App::createSettingBlueprint($this->entityManager, $record);
            ...
            try {
                ...
                $this->logger->error($record['Model'] . ' ' . $e->getMessage());
            }
        } else {
            $io->error($filepath);
