...
#[AsCommand(
    name: 'asphalt:csv:garage:setting:unit',
    description: "Toutes les données pour le Garage",
    aliases: ['asphalt-csv-garage-setting-unit'],
    hidden: false,
)]
class SettingUnitCommand extends Command
{
    ...
    private static string $file       = 'csv.file.garage.setting.unit-price';
    private static string $header     = 'csv.header.garage.setting.unit-price';
    ...
    private function import(string $folder, SymfonyStyle $io): void
    {
        ...
        if ($fs->exists($filepath)) {
            ...
            foreach ($records as $record) {
                ...
                ### Update Entity
                $entity = App::createSettingUnitPrice($this->entityManager, $record);
