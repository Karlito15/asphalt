...
#[AsCommand(
    name: 'asphalt:csv:garage:stat:actual',
    description: "Toutes les données pour le Garage",
    aliases: ['asphalt-csv-garage-stat-actual'],
    hidden: false,
)]
class StatActualCommand extends Command
{
    use ConfigureCommand, InitializeCommand, InteractCommand, QuestionCommand;
    use CSVCommand, FileSystemCommand;

    protected static string $title    = '::::: Garages Stat Actual Datas :::::';

    protected static string $help     = '';

    private static string $database   = 'csv.folders.database.datas';
    private static string $file       = 'csv.file.garage.stat-actual';
    private static string $header     = 'csv.header.garage.stats';
    ...
    private function import(string $folder, SymfonyStyle $io): void
    {
        ...
        if ($fs->exists($filepath)) {
            ...
            foreach ($records as $record) {
                ...
                ### Create Entity
                $entity = App::createStatActual($this->entityManager, $record);
