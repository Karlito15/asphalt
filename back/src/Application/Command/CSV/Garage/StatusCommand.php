...
#[AsCommand(
    name: 'asphalt:csv:garage:status',
    description: "Toutes les données pour le Garage",
    aliases: ['asphalt-csv-garage-status'],
    hidden: false,
)]
class StatusCommand extends Command
{
    ...
    protected static string $title    = '::::: Garages Status Datas :::::';

    protected static string $help     = '';

    private static string $database   = 'csv.folders.database.datas';
    private static string $file       = 'csv.file.garage.status';
    private static string $header     = 'csv.header.garage.status';
    ...
    private function import(string $folder, SymfonyStyle $io): void
    {
        ...
        if ($fs->exists($filepath)) {
            ...
            foreach ($records as $record) {
                ...
                ### Create Entity
                $entity = App::createStatus($this->entityManager, $record);
