...
#[AsCommand(
    name: 'asphalt:csv:garage:setting:level',
    description: "Toutes les données pour le Garage",
    aliases: ['asphalt-csv-garage-setting-level'],
    hidden: false,
)]
class SettingLevelCommand extends Command
{
    ...
    private static string $file       = 'csv.file.garage.setting.level';
    private static string $header     = 'csv.header.garage.setting.level';
    ...
    private function import(string $folder, SymfonyStyle $io): void
    {
        ...
        if ($fs->exists($filepath)) {
            ...
            foreach ($records as $record) {
                ...
                ### Update Entity
                $entity = App::createSettingLevel($this->entityManager, $record);
