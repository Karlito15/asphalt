...
#[AsCommand(
    name: 'asphalt:csv:mission:type',
    description: 'Toutes les données pour les Types',
    aliases: ['asphalt-csv-mission-type'],
    hidden: false,
)]
class TypeCommand extends Command
{
    ...
    protected static string $title    = '::::: Mission Types Datas :::::';

    protected static string $help     = '';

    private static string $database   = 'csv.folders.database.datas';
    private static string $file       = 'csv.file.mission.type';
    private static string $header     = 'csv.header.mission.commons';
    private static string $folderName = 'missions';
...
//        $logger     = $this->logger;
        ...
        if ($choice === 'import') {
            ...
            $io->newLine(2);
            $io->success("Les données pour les Types sont importées");
            $result = true;
        }
        ...
        if ($choice === 'export') {
            ...
            $io->newLine(2);
            $io->success("Les données pour les Types sont exportées");
            $result = true;
        }
