...
final class LevelController extends AbstractController
{
    ...
    private static string $index = 'app.setting.level.index';

    /** @description link to the create page */
    private static string $create = 'app.setting.level.create';
    ...
    {
        $title = $this->translator->trans('app.setting.level.index.title');

        return $this->render('@App/contents/back/setting/level/index.html.twig', [
    ...
    {
        $title    = $this->translator->trans('app.setting.level.create.title');
        ...
        return $this->render('@App/contents/back/setting/level/form.html.twig', [
            'controller_name'   => $title,
            'current_page'      => $request->attributes->get('_route'),
            'breadcrumb'        => ['level1' => $this->translator->trans('app.setting.level.index.title'), 'level2' => $title],
            'links'             => self::getLinksPage(),
            'entities'          => $entities,
            'form'              => $form,
        ]);
    ...
    public function update(Request $request, SettingLevel $entities, EntityManagerInterface $entityManager): Response
    {
        $title = $this->translator->trans('app.setting.level.update.title');
        $form  = $this->createForm(SettingLevelType::class, $entities)->handleRequest($request);
    ...
    public function delete(Request $request, SettingLevel $entities, EntityManagerInterface $entityManager): Response
    {
