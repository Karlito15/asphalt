...
#[ORM\Entity(repositoryClass: AppGarageRepository::class)]
#[ORM\Table(name: 'app_garage')]
...
class AppGarage
{
    ...
    #[ORM\OneToMany(targetEntity: GarageBlueprint::class, mappedBy: 'garage', cascade: ['persist', 'remove'], orphanRemoval: true)]
    private Collection $blueprint;

    #[ORM\OneToMany(targetEntity: GarageBoolean::class, mappedBy: 'garage', cascade: ['persist', 'remove'], orphanRemoval: true)]
    private Collection $boolean;

    #[ORM\OneToMany(targetEntity: GarageRank::class, mappedBy: 'garage', cascade: ['persist', 'remove'], orphanRemoval: true)]
    private Collection $rank;

    #[ORM\OneToMany(targetEntity: GarageStatMax::class, mappedBy: 'garage', cascade: ['persist', 'remove'], orphanRemoval: true)]
    private Collection $statMax;

    #[ORM\OneToMany(targetEntity: GarageStatMin::class, mappedBy: 'garage', cascade: ['persist', 'remove'], orphanRemoval: true)]
    private Collection $statMin;

    #[ORM\OneToMany(targetEntity: GarageUpgrade::class, mappedBy: 'garage', cascade: ['persist', 'remove'], orphanRemoval: true)]
    private Collection $upgrade;

    #[ORM\ManyToOne(targetEntity: SettingBlueprint::class, cascade: ['persist', 'remove'], inversedBy: 'garage')]
    #[Assert\Type(SettingBlueprint::class)]
    private ?SettingBlueprint $settingBlueprint = null;

    #[ORM\ManyToOne(targetEntity: SettingBrand::class, cascade: ['persist', 'remove'], inversedBy: 'garage')]
    #[Assert\Type(SettingBrand::class)]
    private ?SettingBrand $settingBrand = null;

    #[ORM\ManyToOne(targetEntity: SettingClass::class, cascade: ['persist', 'remove'], inversedBy: 'garage')]
    #[Assert\Type(SettingClass::class)]
    private ?SettingClass $settingClass = null;

    #[ORM\ManyToOne(targetEntity: SettingLevel::class, cascade: ['persist', 'remove'], inversedBy: 'garage')]
    #[Assert\Type(SettingLevel::class)]
    private ?SettingLevel $settingLevel = null;

    #[ORM\ManyToOne(targetEntity: SettingUnitPrice::class, cascade: ['persist', 'remove'], inversedBy: 'garage')]
    ...
    /**
     * @return Collection<int, GarageBoolean>
     */
    public function getBoolean(): Collection
    {
        return $this->boolean;
    }

    public function addBoolean(GarageBoolean $boolean): static
    {
        if (!$this->boolean->contains($boolean)) {
            $this->boolean->add($boolean);
            $boolean->setGarage($this);
        }

        return $this;
    }

    public function removeBoolean(GarageBoolean $boolean): static
    {
        if ($this->boolean->removeElement($boolean)) {
            // set the owning side to null (unless already changed)
            if ($boolean->getGarage() === $this) {
                $boolean->setGarage(null);
            }
        }
    ...
    /**
     * @uses For App\Command\CSV\ImportGarageCommand
     * @uses For App\Controller\App\Garage\CreateController
    ...
    {
        /* @var AppGarage $object */
        $object = $args->getObject();
        if ($object instanceof AppGarage) {
            $object->setSlug();
        }
    }

    /**
     * @uses For App\Controller\App\Garage\UpdateController
     * @param LifecycleEventArgs $args
     * @return void
     */
    #[ORM\PreUpdate]
