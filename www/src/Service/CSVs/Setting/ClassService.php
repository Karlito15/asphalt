...
class ClassService implements ServiceInterface
{
    ...
    public function export(SymfonyStyle $io): void
    {
        ...
        try {
            ...
            // Insert each line
            $csv->insertOne(['Label', 'Value', 'ClassOrder', 'CarsNumber', 'Median']);
            foreach ($rows as $row) {
                $csv->insertOne([$row->getLabel(), $row->getValue(), $row->getClassOrder(), $row->getCarsNumber(), $row->getMedian()]);
            }
    ...
    public function createEntity(array $datas): SettingClass
    {
        ...
        $entity
            ...
            ->setClassOrder($datas['ClassOrder'])
            ->setCarsNumber($datas['CarsNumber'])
            ->setMedian($datas['Median'])
        ;
    ...
    public function getCSVFile(): string
    {
        ...
        return $d. (($e === 'dev') ? 'dev--class.csv' : 'class.csv');
    }
}
