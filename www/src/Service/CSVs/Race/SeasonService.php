...
class SeasonService implements ServiceInterface
{
    use ServiceAble;
    use CommandAble;
    ...
    public function export(SymfonyStyle $io): void
    {
        ...
        try {
            $rows = $this->repository->findBy([], ['chapter' => 'ASC']);
            $file = $this->getCSVFile();
            $csv  = self::createCSV($file);
            // Insert each line
            $csv->insertOne(['Chapter', 'Name']);
            foreach ($rows as $row) {
                $csv->insertOne([$row->getChapter(), $row->getName()]);
            }
    ...
    public function getCSVFile(): string
    {
        ...
        return $d. (($e === 'dev') ? 'dev--season.csv' : 'season.csv');
    }
}
